...
// Lists that gained paging after release page from node version 1.2; earlier
// versions keep returning the API's single response.
const showPaging = (op: Operation, extra: Record<string, unknown[]> = {}) => ({ show: { ...show(op).show, ...extra, ...(op.pagination?.since ? { '@version': [{ _cnd: { gte: op.pagination.since } }] } : {}) } });
...
export const productParameters: INodeProperties[] = [
    ...
    ...operations.flatMap((op): INodeProperties[] => {
        ...
        if (['createDestination', 'createEinvoiceDestination', 'updateEinvoiceDestination'].includes(op.id)) fields.push({ displayName: 'Destination Type', name: 'destinationType', type: 'options', default: 'webhook', displayOptions: show(op), options: [{ name: 'S3 Storage', value: 's3' }, { name: 'Webhook', value: 'webhook' }], description: op.id === 'updateEinvoiceDestination' ? 'Choose S3 Storage to replace the stored keys with those in the n8n S3 credential' : 'S3 credentials are stored in an n8n credential, separate from the request body' });
        ...
        const optional = op.query.filter(q => !q.required && !['cursor', 'before', 'offset', 'limit'].includes(q.name) && q.name !== op.pagination?.parameter);
        ...
        if (['xml', 'upload', 'zip'].includes(op.input)) fields.push({ displayName: 'Input Binary Field', name: 'inputBinaryField', type: 'string', default: 'data', displayOptions: op.input === 'xml' ? { show: { ...show(op).show, xmlSource: ['binary'] } } : show(op), description: 'Binary field containing the XML, image or report ZIP file; works with filesystem and external binary storage' });
        ...
        if (op.idempotency) fields.push({ displayName: 'Idempotency Key', name: 'idempotencyKey', type: 'string', default: '', required: ['submitJob', 'submitBatch'].includes(op.id), displayOptions: show(op), description: 'Stable reference for this business operation. Reuse it only when retrying identical input. Public Finance calls cannot use a key.' });
        if (op.pagination) fields.push(
            { displayName: 'Return All', name: 'returnAll', type: 'boolean', default: false, displayOptions: showPaging(op), description: 'Whether to return all results or only up to a given limit' },
            { displayName: 'Limit', name: 'limit', type: 'number', default: 50, typeOptions: { minValue: 1 }, displayOptions: showPaging(op, { returnAll: [false] }), description: 'Max number of results to return' },
            { displayName: { offset: 'Start Offset', since: 'Start After Sequence' }[op.pagination.parameter as string] ?? 'Start Cursor', name: 'pageStart', type: ['offset', 'since'].includes(op.pagination.parameter) ? 'number' : 'string', default: ['offset', 'since'].includes(op.pagination.parameter) ? 0 : '', displayOptions: showPaging(op) },
        );
        return fields;
    }),
];
