...
cleanup() { docker rm -f "$run_name-api" "$run_name-node" >/dev/null 2>&1 || true; docker network rm "$run_name" >/dev/null 2>&1 || true; rm -rf "$fixture_dir"; }
trap cleanup EXIT
node scripts/runtime-fixtures.cjs "$fixture_dir"
docker network create "$run_name" >/dev/null
docker run -d --name "$run_name-api" --network "$run_name" --network-alias fixture-api -v "$PWD/tests/runtime/server.cjs:/server.cjs:ro" -v "$fixture_dir:/output" node:24-alpine node /server.cjs >/dev/null
...
node scripts/runtime-fixtures.cjs "$fixture_dir" verify || { cat "$fixture_dir/execution.log"; docker logs "$run_name-api"; exit 1; }
