...
for(const [baseUrl,expected] of [['https://api.ironfang.uk','https://api.ironfang.uk/v1/screenshot'],['https://api.ironfang.uk/renderwolf///','https://api.ironfang.uk/render/v1/screenshot'],['https://api.ironfang.uk/render','https://api.ironfang.uk/render/v1/screenshot']]) test(`base URL ${baseUrl}`,async()=>{const {ctx}=await run([{}],{baseUrl});assert.equal(ctx.requests[0].url,expected);});
...
for(const [name,statusCode,body,status] of [['usage',200,{limit:250},'OK'],['unmetered',200,{unmetered:true},'OK'],['scoped',403,{error:{code:'insufficient_scope'}},'OK'],['revoked',401,{error:{code:'invalid_api_key'}},'Error'],['firewall',403,'Forbidden','Error'],['bad URL',200,'HTML','Error'],['outage',503,{error:{code:'unavailable'}},'Error']])test(`credential check: ${name}`,async()=>{let request;const result=await credentialTest.call({helpers:{request:async r=>(request=r,{statusCode,body})}},{data:{testProduct:'renderwolf',apiKey:'fixture',baseUrl:'https://api.ironfang.uk/renderwolf/'}});assert.equal(result.status,status);assert.equal(request.uri,'https://api.ironfang.uk/render/v1/usage');assert.equal(request.followRedirect,false);});
