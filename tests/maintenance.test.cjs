...
for(const [name,statusCode,body,status] of [['usage',200,{limit:250},'OK'],['unmetered',200,{unmetered:true},'OK'],['scoped',403,{error:{code:'insufficient_scope'}},'OK'],['revoked',401,{error:{code:'invalid_api_key'}},'Error'],['firewall',403,'Forbidden','Error'],['bad URL',200,'HTML','Error'],['outage',503,{error:{code:'unavailable'}},'Error']])test(`credential check: ${name}`,async()=>{let request;const result=await credentialTest.call({helpers:{request:async r=>(request=r,{statusCode,body})}},{data:{testProduct:'renderwolf',apiKey:'fixture',baseUrl:'https://api.ironfang.uk/renderwolf/'}});assert.equal(result.status,status);assert.equal(request.uri,'https://api.ironfang.uk/renderwolf/v1/usage');assert.equal(request.followRedirect,false);});
