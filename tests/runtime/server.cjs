...
http.createServer(async (req, res) => {
 ...
 const publicRoute = url.pathname.startsWith('/tools/') || url.pathname.startsWith('/render/v1/results/');
 ...
 if (url.pathname === '/finance/v1/einvoices/validate') return json({ outcome: 'invalid', findings: [] });
 if (url.pathname === '/finance/v1/einvoices/results') return json({ results: [{ operation_id: url.searchParams.has('before') ? 'second' : 'first' }], ...(!url.searchParams.has('before') ? { next_cursor: 'first' } : {}), retention_days: 30 });
 if (url.pathname === '/finance/v1/einvoices/rulesets') return json({ rulesets: [] });
 if (url.pathname === '/render/v1/jobs/fixture-job/result') { res.writeHead(302, { ...headers, location: '/render/v1/results/123/signature' }); res.end(); return; }
 if (url.pathname === '/render/v1/site-preview') return binary(Buffer.concat([Buffer.from('--fixture\r\nContent-Disposition: form-data; name="poster"; filename="poster.jpg"\r\nContent-Type: image/jpeg\r\n\r\n'), png, Buffer.from('\r\n--fixture\r\nContent-Disposition: form-data; name="video"; filename="preview.mp4"\r\nContent-Type: video/mp4\r\n\r\n'), png, Buffer.from('\r\n--fixture--\r\n')]), 'multipart/form-data; boundary=fixture');
 if (url.pathname === '/render/v1/jobs' || url.pathname === '/render/v1/batches') return json({ id: 'queued', status: 'queued' }, 202);
 if (url.pathname === '/render/v1/destinations') return json({ id: 'destination' }, 201);
 ...
 if (url.pathname === '/audit/v1/findings') return json({ findings: [{ id: 'finding' }] });
 if (url.pathname === '/audit/v1/sites') return json({ sites: [] });
 if (url.pathname === '/audit/v1/audits') return json({ audits: [{ id: 'audit' }] });
 if (url.pathname === '/rig/v1/runs') return json({ runs: [{ id: 'run' }] });
 if (url.pathname === '/finance/v1/einvoices/reports/verify') return json({ verified: true });
 if (url.pathname === '/audit/v1/sites/fixture-site/audits') return json({ id: 'audit', status: 'queued' }, 202);
